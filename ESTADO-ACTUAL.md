...
Tras una discusión sobre precios, se acordó: bajar el plan anual de 119,99€ a 99,99€ (≈8,33€/mes), subir "Solo nutrición" de 4,99€ a 6,99€/mes, y crear dos planes trimestrales nuevos (Plan completo 39,99€ y Solo nutrición 18,99€) para fidelizar clientes que pagan por trimestres. El usuario pidió además que la lógica de datos (contadores, precios, pagos) se actualizara acorde a estos planes, manteniendo todo en `localStorage` (sin tocar Supabase/Stripe, que sigue aparcado).

- **Tarjetas de precios** (`#pricing`): de 4 a 6 tarjetas (`.pricing-cards` ahora `grid-template-columns: repeat(3, 1fr)`): Primer mes (0,99€, sin cambios), Mensual (14,99€, sin cambios, "Más popular"), **Trimestral** (39,99€, nuevo, "Cada 3 meses · Equivale a 13,33€/mes · Ahorras 5€"), Anual (99,99€, antes 119,99€, "Equivale a 8,33€/mes · Ahorras 80€"), Solo nutrición (6,99€/mes, antes 4,99€), **Solo nutrición trimestral** (18,99€, nuevo, "Equivale a 6,33€/mes · Ahorras 2€"). En móvil sigue colapsando a 1 columna.
- **Hero**: "¿Solo buscas nutrición? También hay plan para ti, desde 4,99€/mes" → "desde 6,99€/mes".
- **Tabla de precios centralizada (`PRECIOS`)**: nuevo objeto con el precio, periodo, días de ciclo (30/90/365) y texto de detalle para cada combinación tipoPlan × periodicidad (mensual/trimestral/anual; "Solo nutrición" no tiene opción anual). Función `preciosPlan(tipoPlan)` para acceder con fallback al plan completo.
- **`userData.periodicidad`** (nuevo campo, 'mensual' por defecto): determina el ciclo de facturación del usuario. `precioPlanActual(tipoPlan, creadoISO, periodicidad)` ahora calcula el precio/detalle según la tabla `PRECIOS` (en el mes de prueba sigue devolviendo siempre 0,99€). `actualizarDiasRestantesPlan()` usa el ciclo de la periodicidad activa (30/90/365 días) en lugar de fijo a 30.
- **Selector de periodicidad**: nuevo grupo de radios "¿Con qué periodicidad quieres pagar?" (Mensual/Trimestral/Anual) en el modal "Cambiar tu plan" (`cp-periodicidad`) y en el modal de fin de prueba/paywall (`pw-periodicidad`). Nueva función `actualizarOpcionesPeriodicidad(tipoPlan, grupoId)` oculta la opción "Anual" cuando el tipo de plan es "Solo nutrición" (no tiene tarifa anual) y reasigna a "Mensual" si estaba seleccionada. `valorPeriodicidad(grupoId)` traduce el radio elegido a la clave usada por `PRECIOS`.
...
Verificado en preview: las 6 tarjetas de precios muestran los importes y textos correctos; en el modal "Cambiar tu plan", elegir "Solo nutrición" oculta la opción "Anual" de periodicidad; `precioPlanActual` devuelve los precios/detalles correctos para las 6 combinaciones tipoPlan × periodicidad (con fallback a mensual para "Solo nutrición" + "Anual"); simulando fin del mes de prueba, el paywall muestra el selector de periodicidad, al elegir "Trimestral" actualiza el precio a 39,99€/trimestre, y al confirmar el pago se guarda `periodicidad: 'trimestral'`, `suscripcionActiva: true` y el contador de días restantes pasa a 90. Sin errores de consola. Cuenta de test restaurada a su estado original.

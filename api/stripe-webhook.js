          ...
          try {
            ...
            if (prof?.email) {
              ...
              if (apiKey) {
                ...
                supabaseAdmin.from('email_log').insert({ tipo: 'renovacion', destinatario: prof.email, asunto: `${primerNombre}, la constancia es tu mejor ejercicio`, html: htmlRenovacion, datos: JSON.stringify({ nombre: prof.nombre, entrenos, racha, semana, resumen: `Renovación: ${entrenos} entrenos, racha ${racha}, semana ${semana}.` }) }).then(() => {}).catch(() => {});
              }
            }
              ...
              if (apiKey) {
                ...
                fetch('https://api.resend.com/emails', {
                  ...
                  body: JSON.stringify({
                    ...
                    html: htmlPagoFallido
                  })
                }).catch(e => console.warn('[stripe-webhook] pago-fallido email error:', e.message));
                supabaseAdmin.from('email_log').insert({ tipo: 'pago_fallido', destinatario: prof.email, asunto: `${primerNombre}, no hemos podido cobrar tu suscripción K-ONE`, html: htmlPagoFallido, datos: JSON.stringify({ nombre: prof.nombre }) }).then(() => {}).catch(() => {});
              }
            }

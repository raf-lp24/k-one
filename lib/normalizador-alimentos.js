...
// Copia exacta de index.html (_EQUIV_TILDE, _NO_ES_ESE_ALIMENTO y _reAlimento).
const _EQUIV_TILDE = { a: '[aáàä]', e: '[eéèë]', i: '[iíìï]', o: '[oóòö]', u: '[uúùü]', n: '[nñ]', c: '[cç]' };
const _NO_ES_ESE_ALIMENTO = {
  calabaza: { antes: '(?<!semillas de )(?<!pipas de )' },
  pan: { antes: '(?!panader|pancet)' },
  // Tortilla de maíz/trigo/integral es un wrap, no huevo.
  tortilla: { despues: '(?![a-záéíóúüñ]*\\s+(?:de\\s+)?(?:ma[ií]z|trigo|integral))' },
  leche: { despues: '(?!\\s+(?:sin lactosa\\s+)?(?:de\\s+)?coco)' },
  nata: { despues: '(?!\\s+(?:de\\s+)?coco)' },
};
function _reAlimento(token) {
  const palabras = _normTexto(token).split(/\s+/).filter(Boolean).map(_raizSinPlural);
  const cuerpo = palabras
    .map((p, i) => {
      const base = p
        .replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
        .replace(/[aeiounc]/g, ch => _EQUIV_TILDE[ch] || ch);
      return i === palabras.length - 1 ? base + '[a-záéíóúüñ]*' : base + '[a-záéíóúüñ]{0,3}';
    })
    .join('\\s+');
  const exc = _NO_ES_ESE_ALIMENTO[palabras.join(' ')] || {};
  return new RegExp('(?<![a-záéíóúüñ])' + (exc.antes || '') + cuerpo + (exc.despues || ''), 'gi');
}
...
function _expandirAlergenoLibre(texto) {
  ...
  const extra = new Set();
  // Palabra a palabra dentro de una frase ("alergia al marisco"), igual que el motor.
  const palabras = s => ' ' + String(s).split(/\s+/).filter(Boolean).map(_normAlimento).join(' ') + ' ';
  tokens.forEach(t => {
    const tNorm = _normAlimento(t);
    const tPalabras = palabras(t);
    Object.entries(SINONIMOS_ALERGENO).forEach(([clave, miembros]) => {
      if ([clave, ...miembros].some(c => _normAlimento(c) === tNorm || tPalabras.includes(palabras(c)))) {
...
// Copia exacta de index.html (ALERGIAS_DEDICADAS, _alergiasDedicadas,
// _quitarAlergiasDedicadas): gluten/lactosa/frutos secos escritos a mano
// activan el mismo sistema que el desplegable.
const ALERGIAS_DEDICADAS = {
  'gluten': ['gluten', 'celiac', 'celiaqu', 'trigo'],
  'lactosa': ['lactosa'],
  'frutos secos': ['frutos secos', 'fruto seco', 'nuez', 'nueces', 'almendra', 'avellana', 'cacahuete', 'anacardo', 'pistacho'],
};
function _alergiasDedicadas(texto, esAlergia = true) {
  const t = _normTexto(texto);
  const hay = k => ALERGIAS_DEDICADAS[k].some(p => t.includes(p));
  return {
    gluten: hay('gluten'),
    lactosa: hay('lactosa') && !(esAlergia && /\bleches?\b|lacteo|caseina|aplv/.test(t)),
    frutosSecos: hay('frutos secos'),
  };
}
function _quitarAlergiasDedicadas(texto, activas) {
  if (!texto) return '';
  return String(texto).split(_SEPARADOR_ALIMENTOS).map(x => x.trim()).filter(Boolean).filter(tok => {
    const n = _normTexto(tok);
    if (activas.gluten && ALERGIAS_DEDICADAS['gluten'].some(p => n.includes(p))) return false;
    if (activas.lactosa && ALERGIAS_DEDICADAS['lactosa'].some(p => n.includes(p))) return false;
    return true;
  }).join(', ');
}
...
function auditarPlan(userData, plan) {
  ...
  // incluidas las alergias escritas a mano ("Otra" y "no como").
  const dedOtra = alergia === 'Otra' ? _alergiasDedicadas(ud.alergiaOtra) : {};
  const dedNoCome = _alergiasDedicadas(ud.noComida, false);
  const conGluten = dieta.includes('gluten') || alergia.includes('Gluten') || !!(dedOtra.gluten || dedNoCome.gluten);
  const conLactosa = dieta.includes('lactosa') || alergia.includes('Lactosa') || !!(dedOtra.lactosa || dedNoCome.lactosa);
  const alergenos = [];
  if (conGluten) alergenos.push('gluten');
  if (conLactosa) alergenos.push('lactosa');
  if (alergia.includes('Frutos secos') || dedOtra.frutosSecos || dedNoCome.frutosSecos) alergenos.push('frutos secos');
  ...
  const activas = { gluten: conGluten, lactosa: conLactosa };
  const otraLimpia = alergia === 'Otra' ? _quitarAlergiasDedicadas(ud.alergiaOtra, activas) : '';
  const alergiaLibre = otraLimpia ? _expandirAlergenoLibre(otraLimpia) : '';
  const noComidaLimpia = _quitarAlergiasDedicadas(ud.noComida, activas);
  ...
  meterTokens(_tokensConOrigen(noComidaLimpia), o => 'no come: ' + o);
...
module.exports = {
  _normTexto, _raizSinPlural, _normAlimento, _reAlimento, _SEPARADOR_ALIMENTOS, _EQUIV_TILDE, _NO_ES_ESE_ALIMENTO,
  _expandirAlergenoLibre, _tokensNoComida, _tokensConOrigen,
  ALERGIAS_DEDICADAS, _alergiasDedicadas, _quitarAlergiasDedicadas,
